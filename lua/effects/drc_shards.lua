function EFFECT:Init( data )
	...
	for i = 0,math.Rand(5,math.Clamp(self.Magnitude, 5, 150)) do
		...
		local particle = emitter:Add( "effects/fleck_glass1", Pos)
		if particle == nil then particle = emitter2:Add( "effects/fleck_glass2", Pos) end
		if (particle) then
			local chance = math.Round(math.Rand(1,3))
			if chance == 1 then
				particle:SetMaterial("effects/fleck_glass1")
			elseif chance == 2 then
				particle:SetMaterial("effects/fleck_glass2")
			else
				particle:SetMaterial("effects/fleck_glass3")
			end
